...
const char *saveimg_filename = "~/catkin_ws/src/imgpcl/obj_resullt_img/";
CvFont font;
CvScalar textColor;
char text[200];
...
{   	   
	...
	ifstream fin("~/catkin_ws/src/imgpcl/template/ed_final_list.txt");   //注意注意-------如果复制到其他地方，修改目录
	for(int i=0;i<temp_num;i++) //获取三个模板的直方图；
    {
        char name[100];
        fin>>name; 
        char tempName[255];
        sprintf(tempName, "~/catkin_ws/src/imgpcl/template/%s", name);  
...
void callback(const ImageConstPtr& image, const PointCloud2::ConstPtr& Callback_Points)
{
    ...
    if(is_find)
    {
		...
		fstream ofsp("~/catkin_ws/src/imgpcl/config/pos.txt",ofstream::out);    
        ofsp<<p.x;
        ofsp<<p.y;
        ofsp<<p.z;  
        ofsp<<'\n';  
		...
		fstream ofsnewp("~/catkin_ws/src/imgpcl/config/newpos.txt",ofstream::out);  
        ofsnewp<<newp.x;
        ofsnewp<<newp.y;
        ofsnewp<<newp.z;  
        ofsnewp<<'\n';  
