...
cout<<rc.y-2<<endl;
cout<<rc.x+rc.width - 2<<endl;
cout<<rc.y-2<<endl;
cout<<rc.y+rc.height-2<<endl;
	int x0 = 0;
	int x1 = 0;
	...
	if(rc.x+2 >= 0)
		x0 = rc.x+2 ;
	if(rc.x+rc.width-2 >= 640)
		x1 = 639;
	if(rc.x+rc.width-2 < 640)
		x1 = rc.x+rc.width-2;
	if(rc.y+rc.height >= 480)
	    y1 = 479;
    if(rc.y+rc.height < 480)
	    y1 = rc.y+rc.height;
	for(int i=x0; i <= x1; i++)//考虑到深度摄像头与彩色摄像头不在同一位置，所以添加两个像素
	{
	    for(int j=rc.y +2; j<= y1; j++)
	    {
